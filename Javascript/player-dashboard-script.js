...
async function handlePendingAttack(attackData, playerRef) {
    ...
    Swal.fire(swalOptions).then(async (result) => {
        ...
        if (result.isConfirmed) { 
            ...
            if (damageTaken <= 0) {
                feedbackTitle = '🛡️ ป้องกันสมบูรณ์!';
                ...
                feedbackTitle = '🛡️ ป้องกันได้บางส่วน';
            ...
            if (isDodgeSuccess) {
                feedbackTitle = '🏃 หลบพ้น!';
                feedbackHtml = `พริ้วไหว! (${totalDodge} vs ${attackData.attackRollValue})`;
                defenseResponse.damageTaken = 0;
                defenseResponse.success = true;
            } else {
                feedbackTitle = '🏃 หลบไม่พ้น!';
                feedbackHtml = `สะดุด! (${totalDodge} vs ${attackData.attackRollValue})<br>โดน <strong>${initialDamage}</strong>`;
